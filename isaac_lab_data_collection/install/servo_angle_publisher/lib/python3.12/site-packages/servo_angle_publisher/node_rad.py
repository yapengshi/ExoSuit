...
DEG_TO_RAD = math.pi / 180.0  # Added: JointState expects radians, keep internal accumulation in degrees. (EN)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if publishing:
                ...
                # Added: convert degrees to radians for JointState (robot_state_publisher expects radians). (EN)
                logical_left_rad = [d * DEG_TO_RAD for d in logical_left]   # Added (EN)
                logical_right_rad = [d * DEG_TO_RAD for d in logical_right] # Added (EN)

                joint_msg = JointState()
                joint_msg.header.stamp = rospy.Time.now()
                joint_msg.name = joint_names
                # Fixed: keep position order aligned with joint_msg.name (right arm first, then left arm). (EN)
                joint_msg.position = logical_right_rad + logical_left_rad   # Changed (EN)
