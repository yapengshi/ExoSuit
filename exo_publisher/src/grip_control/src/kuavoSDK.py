...
from dynamic_biped.msg import walkCommand        # 行走速度设置
from sensor_msgs.msg import JointState           # /kuavo_arm_traj
...
from dynamic_biped.msg import robotArmInfo       # 机器人手臂state 包含 q v vd tau
from dynamic_biped.msg import robotHandPosition    # 机器人手掌当前的12关节手指位置数据 
from dynamic_biped.msg import robotHeadMotionData  # 机器人头部数据
...
class kuavo:
    ...
    def __init__(self, name):
        ...
        self.latest_robot_arm_state = None
        self.latest_robot_handP = None
        self.latest_robot_headP = None
        ...
        self._robot_arm_qvtau_sub = rospy.Subscriber("/robot_arm_q_v_tau", robotArmInfo, self.get_robot_arm_qvtau_callback)
        self._robot_handP_sub = rospy.Subscriber("/robot_hand_position", robotHandPosition, self.get_robot_handPosition_callback)
        self._robot_headP_sub = rospy.Subscriber("/robot_head_motion_data", robotHeadMotionData, self.get_robot_headPosition_callback)
        ...
        self._robot_control_end_client = rospy.ServiceProxy("/control_end_hand", controlEndHand)
    ...
    def get_robot_arm_qvtau_callback(self, msg):
        self.latest_robot_arm_state = msg
    
    def get_robot_handPosition_callback(self, msg):
        self.latest_robot_handP = msg
    
    def get_robot_headPosition_callback(self, msg):
        self.latest_robot_headP = msg
    ...
    def srv_ChangectlMode(self, _master_id:int, _control_mode:int)->int:
        try:
            ...
            return 0
    def srv_Control_end_call(self, l_hand_p, r_hand_p)->bool:
        try:
            # 客户端发送requets
            request = controlEndHandRequest()
            request.left_hand_position = l_hand_p
            request.right_hand_position = r_hand_p
            
            # 客户端接受response
            response = self._robot_control_end_client(request)
            
            return response.result
        
        except rospy.ServiceException as e:
            rospy.logerr(f"srv_Control_end_call Service call failed: {e}")
            return False
    ...
    def set_walk_speed(self, control_mode:int, v_x:float, v_y:float, v_yaw:float, v_step=0):
        ...
        if control_mode == 1:
            # 速度模式控制
            # model_logger.log_info("....VelocityCommand  ", " v_x : ", v_x , " v_y : ", v_y ," v_yaw : ", v_yaw)
            self.pub_kuavo_walk_speed(control_mode, v_x, v_y, v_yaw)
        elif control_mode == 2:
            # 步态模式控制
            # model_logger.log_info("....stepCommand v_step: ", v_step, " v_x : ", v_x , " v_y : ", v_y ," v_yaw : ", v_yaw)
    ...
    def set_robot_arm_recenter(self)->bool:
        ...
        try:
            ...
            # model_logger.log_info(f"An error occurred: {e}")
    ...
    def set_robot_end_position_control(self, l_hand_position:int, r_hand_position:int)->bool:
        """设置机器人手掌的末端位置

        Args:
            l_hand_position (int): 左手掌的6自由度数据 uint8[] 维度为6
            r_hand_position (int): 右手掌的6自由度数据 uint8[] 维度为6

        Returns:
            bool: 是否调用成功, 成功返回true, 失败调取Flase
        """
        try:
            self.srv_Control_end_call(l_hand_position, r_hand_position)
        except Exception as e:
            # model_logger.log_info(f"An error occurred: {e}")      
            print("An error occurred: {e}")  
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
       result = robot_instance.get_QVTau()
       time.sleep(1)
    #    model_logger.log_info(result)
