...
# ---- Added: Angle limits (in radians) ----
_arm_min_deg = [-180, -20, -135, -100, -135, -45, -45, -180, -180, -180, -180, -180, -45, -45]
_arm_max_deg = [  30, 135,  135,  100,  135,  45,  45,  180,  180,  180,  180,  180,  45,  45]
ARM_MIN_RAD = list(map(math.radians, _arm_min_deg))
ARM_MAX_RAD = list(map(math.radians, _arm_max_deg))

def _clamp_positions(pos):
    return [max(min(p, ARM_MAX_RAD[i]), ARM_MIN_RAD[i]) for i, p in enumerate(pos)]

# ---- Added: Exponential angle filter ----
class AngleFilter:
    def __init__(self, alpha, datalen):
        self.alpha = float(alpha)
        self.data = [0.0] * int(datalen)
    def filter(self, new_vals):
        for i in range(len(self.data)):
            self.data[i] = self.alpha * new_vals[i] + (1.0 - self.alpha) * self.data[i]
        return self.data[:]
...
class FeetechMotor:
    ...
    def read_positions(self, motor_ids):
        addr, bytes_len = SCS_SERIES_CONTROL_TABLE["Present_Position"]
        group = scs.GroupSyncRead(self.port_handler, self.packet_handler, addr, bytes_len)
        for idx in motor_ids:
            group.addParam(idx)
        comm = group.txRxPacket()
        if comm != scs.COMM_SUCCESS:
            raise ConnectionError(f"Read failed: {self.packet_handler.getTxRxResult(comm)}")
        positions = [group.getData(idx, addr, bytes_len) for idx in motor_ids]
        return positions
...
if __name__ == "__main__":
    ...
    pub = rospy.Publisher("/kuavo_arm_traj", JointState, queue_size=10)  # /joint_states #/kuavo_arm_traj
    rate = rospy.Rate(10)
    ...
    joint_names = [
        ...
        "arm_joint_7", "arm_joint_8", "arm_joint_9", "arm_joint_10",    
        "arm_joint_11", "arm_joint_12", "arm_joint_13"
    ]
    ...
    # ---- Added: instantiate filter for 14 joints ----
    angle_filter = AngleFilter(alpha=0.2, datalen=len(joint_names))
    ...
    try:
        while not rospy.is_shutdown():
            ...
            raw_positions = accumulated_radians_left + accumulated_radians_right

            # ---- Added: filtering + clamping before publish ----
            filtered_positions = angle_filter.filter(raw_positions)
            safe_positions = _clamp_positions(filtered_positions)

            joint_msg = JointState()
            joint_msg.header.stamp = rospy.Time.now()
            joint_msg.name = joint_names
            joint_msg.position = [round(p, 1) for p in safe_positions]
